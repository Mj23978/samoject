...
enum SamojectTableResizeMode {
  none,
  normal,
  pushAndPull;

  bool get isNone => this == SamojectTableResizeMode.none;
  bool get isNormal => this == SamojectTableResizeMode.normal;
  bool get isPushAndPull => this == SamojectTableResizeMode.pushAndPull;
}
...
enum SamojectTableAutoSizeMode {
  none,
  equal,
  scale;

  bool get isNone => this == SamojectTableAutoSizeMode.none;
  bool get isEqual => this == SamojectTableAutoSizeMode.equal;
  bool get isScale => this == SamojectTableAutoSizeMode.scale;
}

/// Returns the auto-sizing class according to
/// [SamojectTableAutoSizeMode.equal] or [SamojectTableAutoSizeMode.scale].
///
...
/// If [mode] is [SamojectTableAutoSizeMode.scale], you must pass the value of [scale].
...
/// {@endtemplate}
class SamojectTableAutoSizeHelper {
  static SamojectTableAutoSize items({
    required double maxSize,
    required int length,
    required double itemMinSize,
    required SamojectTableAutoSizeMode mode,
    double? scale,
  }) {
    if (mode.isScale) assert(scale != null);

    switch (mode) {
      case SamojectTableAutoSizeMode.equal:
        return SamojectTableAutoSizeEqual(
          maxSize: maxSize,
          length: length,
          itemMinSize: itemMinSize,
        );
      case SamojectTableAutoSizeMode.scale:
        return SamojectTableAutoSizeScale(
          maxSize: maxSize,
          length: length,
          scale: scale!,
          itemMinSize: itemMinSize,
        );
      case SamojectTableAutoSizeMode.none:
        throw Exception(
            'Mode cannot be called with SamojectTableAutoSizeMode.none.');
    }
  }
}

abstract class SamojectTableAutoSize {
...
class SamojectTableAutoSizeEqual implements SamojectTableAutoSize {
  SamojectTableAutoSizeEqual({
...
class SamojectTableAutoSizeScale implements SamojectTableAutoSize {
  SamojectTableAutoSizeScale({
    required this.maxSize,
    required this.length,
    required this.scale,
    required this.itemMinSize,
...
///
/// Cannot be called with [SamojectTableResizeMode.none] or [SamojectTableResizeMode.normal] .
///
...
/// {@endtemplate}
class SamojectTableResizeHelper {
  static SamojectTableResize items<T>({
    ...
    required SamojectTableResizeMode mode,
  }) {
    switch (mode) {
      case SamojectTableResizeMode.pushAndPull:
        return SamojectTableResizePushAndPull<T>(
          offset: offset,
          items: items,
      ...
      case SamojectTableResizeMode.none:
      case SamojectTableResizeMode.normal:
...
/// that must be inherited when implementing the class according to [SamojectTableResizeMode].
///
/// {@macro resize_helper_params}
abstract class SamojectTableResize<T> {
  SamojectTableResize({
...
class SamojectTableResizePushAndPull<T> extends SamojectTableResize<T> {
  SamojectTableResizePushAndPull({
