...
abstract class ISelectingState {
  ...
  /// [selectingMode]
  SamojectTableGridSelectingMode get selectingMode;

  /// Current position of multi-select cell.
  /// Calculate the currently selected cell and its multi-selection range.
  SamojectTableGridCellPosition? get currentSelectingPosition;

  /// Position list of currently selected.
  /// Only valid in [SamojectTableGridSelectingMode.cell].
  ///
  /// ```dart
  ...
  /// });
  /// ```
  List<SamojectTableGridSelectingCellPosition> get currentSelectingPositionList;

  bool get hasCurrentSelectingPosition;

  /// Rows of currently selected.
  /// Only valid in [SamojectTableGridSelectingMode.row].
  List<SamojectTableRow> get currentSelectingRows;
  ...
  void setSelectingMode(SamojectTableGridSelectingMode mode,
      {bool notify = true});
  ...
  bool isSelectedCell(
      SamojectTableCell cell, SamojectTableColumn column, int rowIdx);

  /// The action that is selected in the Select dialog
  /// and processed after the dialog is closed.
  void handleAfterSelectingRow(SamojectTableCell cell, dynamic value);
}

mixin SelectingState implements ISamojectTableGridState {
  @override
  ...
  @override
  SamojectTableGridSelectingMode get selectingMode => _selectingMode;

  SamojectTableGridSelectingMode _selectingMode =
      SamojectTableGridSelectingMode.cell;

  @override
  SamojectTableGridCellPosition? get currentSelectingPosition =>
      _currentSelectingPosition;

  SamojectTableGridCellPosition? _currentSelectingPosition;

  @override
  List<SamojectTableGridSelectingCellPosition>
      get currentSelectingPositionList {
    if (currentCellPosition == null || currentSelectingPosition == null) {
      return [];
    }

    switch (_selectingMode) {
      case SamojectTableGridSelectingMode.cell:
        return _selectingCells();
      case SamojectTableGridSelectingMode.horizontal:
        return _selectingCellsHorizontally();
      case SamojectTableGridSelectingMode.row:
      case SamojectTableGridSelectingMode.none:
        return [];
    }
  }
  ...
  @override
  List<SamojectTableRow> get currentSelectingRows => _currentSelectingRows;

  List<SamojectTableRow> _currentSelectingRows = [];
  ...
  void setAllCurrentSelecting() {
    ...
    switch (_selectingMode) {
      ...
      case SamojectTableGridSelectingMode.row:
        ...
        _currentSelectingPosition = SamojectTableGridCellPosition(
          columnIdx: refColumns.length - 1,
          rowIdx: refRows.length - 1,
        );
  ...
  void toggleSelectingRow(int? rowIdx, {notify = true}) {
    ...
    final SamojectTableRow row = refRows[rowIdx];
  ...
  void handleAfterSelectingRow(SamojectTableCell cell, dynamic value) {
    ...
    if (configuration.enableMoveDownAfterSelecting) {
      moveCurrentCell(SamojectTableMoveDirection.down, notify: false);
  ...
  List<SamojectTableGridSelectingCellPosition> _selectingCells() {
    final List<SamojectTableGridSelectingCellPosition> positions = [];
    ...
    for (int i = rowStartIdx; i <= rowEndIdx; i += 1) {
      for (int j = columnStartIdx; j <= columnEndIdx; j += 1) {
        ...
        positions.add(SamojectTableGridSelectingCellPosition(
          rowIdx: i,
          field: field,
        ));
      }
  ...
  List<SamojectTableGridSelectingCellPosition> _selectingCellsHorizontally() {
    final List<SamojectTableGridSelectingCellPosition> positions = [];
    ...
    SamojectTableGridCellPosition startCell =
        firstCurrent ? currentCellPosition! : currentSelectingPosition!;

    SamojectTableGridCellPosition endCell =
        !firstCurrent ? currentCellPosition! : currentSelectingPosition!;
