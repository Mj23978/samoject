...
abstract class IEditingState {
  ...
  dynamic castValueByColumnType(dynamic value, SamojectTableColumn column);

  /// Change cell value
  /// [callOnChangedEvent] triggers a [SamojectTableOnChangedEventCallback] callback.
...
mixin EditingState implements ISamojectTableGridState {
  @override
  ...
  dynamic castValueByColumnType(dynamic value, SamojectTableColumn column) {
    if (column.type is SamojectTableColumnTypeWithNumberFormat) {
      return (column.type as SamojectTableColumnTypeWithNumberFormat)
  ...
  void changeCellValue(
    ...
    currentRow.setState(SamojectTableRowState.updated);

    cell.value = value;

    if (callOnChangedEvent == true && onChanged != null) {
      onChanged!(SamojectTableGridOnChangedEvent(
  ...
  void _pasteCellValueInOrder({
    ...
    for (int i = 0; i < rowIdxList.length; i += 1) {
      ...
      for (int columnIdx = columnStartIdx!;
        ...
        refRows[rowIdx].setState(SamojectTableRowState.updated);
