...
abstract class IRowState {
  List<SamojectTableRow> get rows;

  FilteredList<SamojectTableRow> refRows = FilteredList();

  List<SamojectTableRow> get checkedRows;

  List<SamojectTableRow> get unCheckedRows;
  ...
  SamojectTableRow? get currentRow;

  SamojectTableRowColorCallback? get rowColorCallback;

  int? getRowIdxByOffset(double offset);

  SamojectTableRow? getRowByIdx(int rowIdx);

  SamojectTableRow getNewRow();

  List<SamojectTableRow> getNewRows({
    int count = 1,
  });

  List<SamojectTableRow> setSortIdxOfRows(
  ...
  void prependRows(List<SamojectTableRow> rows);

  void appendNewRows({
    int count = 1,
  });

  void appendRows(List<SamojectTableRow> rows);

  void removeCurrentRow();

  void removeRows(List<SamojectTableRow> rows);
  ...
  void setRowColorCallback(SamojectTableRowColorCallback rowColorCallback);
}

mixin RowState implements ISamojectTableGridState {
  @override
  List<SamojectTableRow> get rows => [...refRows];

  @override
  FilteredList<SamojectTableRow> get refRows => _refRows;

  @override
  set refRows(FilteredList<SamojectTableRow> setRows) {
    SamojectTableGridStateManager.initializeRows(
        refColumns.originalList, setRows);
    _refRows = setRows;
  }

  FilteredList<SamojectTableRow> _refRows = FilteredList();
  ...
  SamojectTableRowColorCallback? _rowColorCallback;
  ...
  @override
  SamojectTableRow getNewRow() {
    final cells = <String, SamojectTableCell>{};

    for (var column in refColumns) {
      cells[column.field] = SamojectTableCell(
        value: column.type.defaultValue,
      );
    }

    return SamojectTableRow(cells: cells);
  }
  ...
  void insertRows(
    ...
    if (hasSortedColumn) {
      ...
      _insertRows(rowIdx, rows, state: SamojectTableRowState.added);
    } else {
      _insertRows(rowIdx, rows, state: SamojectTableRowState.added);
  ...
  void prependRows(List<SamojectTableRow> rows) {
    ...
    _insertRows(0, rows, state: SamojectTableRowState.added);
    ...
    if (currentCell != null) {
      ...
      scrollByDirection(SamojectTableMoveDirection.up, offsetToMove);
    }
  ...
  void appendRows(List<SamojectTableRow> rows) {
    ...
    _insertRows(refRows.length, rows, state: SamojectTableRowState.added);
  ...
  void moveRowsByIndex(
    ...
    if (originalRowIdx >= refRows.originalLength) {
      refRows.addAll(rows.cast<SamojectTableRow>());
    } else {
      refRows.insertAll(indexToMove, rows.cast<SamojectTableRow>());
    }
    ...
    if (onRowsMoved != null) {
      onRowsMoved!(SamojectTableGridOnRowsMovedEvent(
        idx: indexToMove,
        rows: rows,
      ));
    }
  ...
  void _insertRows(
    ...
    if (originalRowIdx >= refRows.originalLength) {
      ...
      refRows.insertAll(index, rows.cast<SamojectTableRow>());
    }
