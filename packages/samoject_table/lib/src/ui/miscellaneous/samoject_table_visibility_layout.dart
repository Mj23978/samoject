...
/// of [SamojectTableCell] or [SamojectTableColumn], [SamojectTableColumnGroup] of [SamojectTableRow]
/// or render only the widgets displayed according to the screen width.
class SamojectTableVisibilityLayout extends RenderObjectWidget
    implements MultiChildRenderObjectWidget {
  const SamojectTableVisibilityLayout({
    super.key,
  ...
  @override
  final List<SamojectTableVisibilityLayoutId> children;
  ...
  @override
  SamojectTableVisibilityLayoutRenderObjectElement createElement() =>
      SamojectTableVisibilityLayoutRenderObjectElement(
        widget: this,
...
class SamojectTableVisibilityLayoutRenderObjectElement
    extends RenderObjectElement implements MultiChildRenderObjectElement {
  SamojectTableVisibilityLayoutRenderObjectElement({
    required SamojectTableVisibilityLayout widget,
    required this.scrollController,
    this.initialViewportDimension = 1920,
    this.textDirection = TextDirection.ltr,
  ...
  Iterable<SamojectTableVisibilityLayoutId> get _widgetChildren {
    if (textDirection == TextDirection.ltr) {
      return (widget as SamojectTableVisibilityLayout).children;
    }

    return (widget as SamojectTableVisibilityLayout).children.reversed;
  }
  ...
  bool visible({
    ...
    required SamojectTableVisibilityLayoutChild layoutChild,
  }) {
  ...
  Element? findChildByLayoutId(Object layoutId) {
    return _children.firstWhereOrNull((element) {
      if (element.widget is SamojectTableVisibilityLayoutId) {
        return (element.widget as SamojectTableVisibilityLayoutId).id ==
            layoutId;
      }
      return false;
    });
  ...
  void performRebuild() {
    ...
    for (final child in _children) {
      ...
      final layoutChild =
          (child.widget as SamojectTableVisibilityLayoutId).layoutChild;
  ...
  @override
  void update(SamojectTableVisibilityLayout newWidget) {
...
class SamojectTableVisibilityLayoutId extends LayoutId {
  SamojectTableVisibilityLayoutId({
    Key? key,
    required super.id,
    required SamojectTableVisibilityLayoutChild child,
  }) : super(key: key, child: child);

  SamojectTableVisibilityLayoutChild get layoutChild =>
      child as SamojectTableVisibilityLayoutChild;
}

abstract class SamojectTableVisibilityLayoutChild implements Widget {
  double get width;
