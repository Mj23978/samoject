...
mixin PopupCellState<T extends PopupCell> on State<T>
  ...
  /// Implement a callback function that takes [SamojectTableGridStateManager] as a parameter.
    ...
    SamojectTableGridPopup(
      ...
      configuration: widget.stateManager.configuration.copyWith(
        style: widget.stateManager.configuration.style.copyWith(
          ...
          defaultCellPadding: SamojectTableGridSettings.cellPadding,
  ...
  void onLoaded(SamojectTableGridOnLoadedEvent event) {
    ...
    if (event.stateManager.currentRowIdx != null) {
      ...
      if (rowIdxToMove < event.stateManager.refRows.length) {
        event.stateManager
            .moveScrollByRow(SamojectTableMoveDirection.up, rowIdxToMove);
      } else {
        event.stateManager.moveScrollByRow(
            SamojectTableMoveDirection.up, event.stateManager.refRows.length);
      }
    }
  }

  void onSelected(SamojectTableGridOnSelectedEvent event) {
    isOpenedPopup = false;
