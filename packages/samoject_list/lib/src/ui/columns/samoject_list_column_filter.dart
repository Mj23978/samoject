...
import '../../manager/event/samoject_list_grid_change_column_filter_event.dart';
...
class SamojectListColumnFilter extends SamojectListStatefulWidget {
  final SamojectListGridStateManager stateManager;

  final SamojectListColumn column;

  SamojectListColumnFilter({
  ...
  @override
  SamojectListColumnFilterState createState() =>
      SamojectListColumnFilterState();
}

class SamojectListColumnFilterState
    extends SamojectListStateWithChange<SamojectListColumnFilter> {
  ...
  @override
  void updateState() {
    _filterRows = update<List<SamojectListRow>>(
      _filterRows,
  ...
  void _moveDown({required bool focusToPreviousCell}) {
    ...
    if (!focusToPreviousCell || stateManager.currentCell == null) {
      ...
      stateManager.scrollByDirection(SamojectListMoveDirection.down, 0);
    }
  ...
  void _handleFocusFromRows(SamojectListGridEvent tableEvent) {
    if (!_enabled) {
      return;
    }
