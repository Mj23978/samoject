...
/// of [SamojectListCell] or [SamojectListColumn], [SamojectListColumnGroup] of [SamojectListRow]
/// or render only the widgets displayed according to the screen width.
class SamojectListVisibilityLayout extends RenderObjectWidget
    implements MultiChildRenderObjectWidget {
  const SamojectListVisibilityLayout({
    super.key,
  ...
  @override
  final List<SamojectListVisibilityLayoutId> children;
  ...
  @override
  SamojectListVisibilityLayoutRenderObjectElement createElement() =>
      SamojectListVisibilityLayoutRenderObjectElement(
        widget: this,
...
class SamojectListVisibilityLayoutRenderObjectElement extends RenderObjectElement
    implements MultiChildRenderObjectElement {
  SamojectListVisibilityLayoutRenderObjectElement({
    required SamojectListVisibilityLayout widget,
    required this.scrollController,
    this.initialViewportDimension = 1920,
    this.textDirection = TextDirection.ltr,
  ...
  Iterable<SamojectListVisibilityLayoutId> get _widgetChildren {
    if (textDirection == TextDirection.ltr) {
      return (widget as SamojectListVisibilityLayout).children;
    }

    return (widget as SamojectListVisibilityLayout).children.reversed;
  }
  ...
  bool visible({
    ...
    required SamojectListVisibilityLayoutChild layoutChild,
  }) {
  ...
  Element? findChildByLayoutId(Object layoutId) {
    return _children.firstWhereOrNull((element) {
      if (element.widget is SamojectListVisibilityLayoutId) {
        return (element.widget as SamojectListVisibilityLayoutId).id == layoutId;
      }
      return false;
    });
  }
  ...
  void performRebuild() {
    ...
    for (final child in _children) {
      ...
      final layoutChild = (child.widget as SamojectListVisibilityLayoutId).layoutChild;
  ...
  @override
  void update(SamojectListVisibilityLayout newWidget) {
...
class SamojectListVisibilityLayoutId extends LayoutId {
  SamojectListVisibilityLayoutId({
    Key? key,
    required super.id,
    required SamojectListVisibilityLayoutChild child,
  }) : super(key: key, child: child);

  SamojectListVisibilityLayoutChild get layoutChild =>
      child as SamojectListVisibilityLayoutChild;
}

abstract class SamojectListVisibilityLayoutChild implements Widget {
  double get width;
