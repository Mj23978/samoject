...
abstract class ISelectingState {
  ...
  /// [selectingMode]
  SamojectListGridSelectingMode get selectingMode;

  /// Current position of multi-select cell.
  /// Calculate the currently selected cell and its multi-selection range.
  SamojectListGridCellPosition? get currentSelectingPosition;

  /// Position list of currently selected.
  /// Only valid in [SamojectListGridSelectingMode.cell].
  ///
  /// ```dart
  ...
  /// });
  /// ```
  List<SamojectListGridSelectingCellPosition> get currentSelectingPositionList;

  bool get hasCurrentSelectingPosition;

  /// Rows of currently selected.
  /// Only valid in [SamojectListGridSelectingMode.row].
  List<SamojectListRow> get currentSelectingRows;
  ...
  void setSelectingMode(SamojectListGridSelectingMode mode,
      {bool notify = true});
  ...
  bool isSelectedCell(
      SamojectListCell cell, SamojectListColumn column, int rowIdx);

  /// The action that is selected in the Select dialog
  /// and processed after the dialog is closed.
  void handleAfterSelectingRow(SamojectListCell cell, dynamic value);
}

mixin SelectingState implements ISamojectListGridState {
  @override
  ...
  @override
  SamojectListGridSelectingMode get selectingMode => _selectingMode;

  SamojectListGridSelectingMode _selectingMode =
      SamojectListGridSelectingMode.cell;

  @override
  SamojectListGridCellPosition? get currentSelectingPosition =>
      _currentSelectingPosition;

  SamojectListGridCellPosition? _currentSelectingPosition;
  ...
  List<SamojectListGridSelectingCellPosition> get currentSelectingPositionList {
    ...
    switch (_selectingMode) {
      case SamojectListGridSelectingMode.cell:
        return _selectingCells();
      case SamojectListGridSelectingMode.horizontal:
        return _selectingCellsHorizontally();
      case SamojectListGridSelectingMode.row:
      case SamojectListGridSelectingMode.none:
        return [];
    }
  }
  ...
  @override
  List<SamojectListRow> get currentSelectingRows => _currentSelectingRows;

  List<SamojectListRow> _currentSelectingRows = [];
  ...
  void setAllCurrentSelecting() {
    ...
    switch (_selectingMode) {
      ...
      case SamojectListGridSelectingMode.row:
        ...
        _currentSelectingPosition = SamojectListGridCellPosition(
          columnIdx: refColumns.length - 1,
          rowIdx: refRows.length - 1,
        );
  ...
  void toggleSelectingRow(int? rowIdx, {notify = true}) {
    ...
    final SamojectListRow row = refRows[rowIdx];
  ...
  void handleAfterSelectingRow(SamojectListCell cell, dynamic value) {
    ...
    if (configuration.enableMoveDownAfterSelecting) {
      moveCurrentCell(SamojectListMoveDirection.down, notify: false);
  ...
  List<SamojectListGridSelectingCellPosition> _selectingCells() {
    final List<SamojectListGridSelectingCellPosition> positions = [];
    ...
    for (int i = rowStartIdx; i <= rowEndIdx; i += 1) {
      for (int j = columnStartIdx; j <= columnEndIdx; j += 1) {
        ...
        positions.add(SamojectListGridSelectingCellPosition(
          rowIdx: i,
          field: field,
        ));
      }
  ...
  List<SamojectListGridSelectingCellPosition> _selectingCellsHorizontally() {
    final List<SamojectListGridSelectingCellPosition> positions = [];
    ...
    SamojectListGridCellPosition startCell =
        firstCurrent ? currentCellPosition! : currentSelectingPosition!;

    SamojectListGridCellPosition endCell =
        !firstCurrent ? currentCellPosition! : currentSelectingPosition!;
