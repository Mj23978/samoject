...
abstract class IEditingState {
  ...
  dynamic castValueByColumnType(dynamic value, SamojectListColumn column);

  /// Change cell value
  /// [callOnChangedEvent] triggers a [SamojectListOnChangedEventCallback] callback.
...
mixin EditingState implements ISamojectListGridState {
  @override
  ...
  dynamic castValueByColumnType(dynamic value, SamojectListColumn column) {
    if (column.type is SamojectListColumnTypeWithNumberFormat) {
      return (column.type as SamojectListColumnTypeWithNumberFormat)
  ...
  void changeCellValue(
    ...
    currentRow.setState(SamojectListRowState.updated);

    cell.value = value;

    if (callOnChangedEvent == true && onChanged != null) {
      onChanged!(SamojectListGridOnChangedEvent(
  ...
  void _pasteCellValueInOrder({
    ...
    for (int i = 0; i < rowIdxList.length; i += 1) {
      ...
      for (int columnIdx = columnStartIdx!;
        ...
        refRows[rowIdx].setState(SamojectListRowState.updated);
