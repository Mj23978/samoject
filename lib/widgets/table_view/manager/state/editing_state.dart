...
abstract class IEditingState {
  ...
  dynamic castValueByColumnType(dynamic value, TableViewColumn column);

  /// Change cell value
  /// [callOnChangedEvent] triggers a [TableOnChangedEventCallback] callback.
...
mixin EditingState implements ITableGridState {
  @override
  ...
  dynamic castValueByColumnType(dynamic value, TableViewColumn column) {
    if (column.type is TableColumnTypeWithNumberFormat) {
      return (column.type as TableColumnTypeWithNumberFormat)
  ...
  void changeCellValue(
    ...
    currentRow.setState(TableRowState.updated);

    cell.value = value;

    if (callOnChangedEvent == true && onChanged != null) {
      onChanged!(TableGridOnChangedEvent(
  ...
  void _pasteCellValueInOrder({
    ...
    for (int i = 0; i < rowIdxList.length; i += 1) {
      ...
      for (int columnIdx = columnStartIdx!;
        ...
        refRows[rowIdx].setState(TableRowState.updated);
