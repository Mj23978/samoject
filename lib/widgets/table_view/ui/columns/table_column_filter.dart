...
import '../../manager/event/table_grid_change_column_filter_event.dart';
...
class TableColumnFilter extends TableStatefulWidget {
  ...
  @override
  TableColumnFilterState createState() => TableColumnFilterState();
}

class TableColumnFilterState extends TableStateWithChange<TableColumnFilter> {
  ...
  @override
  void updateState() {
    _filterRows = update<List<TableViewRow>>(
      _filterRows,
  ...
  void _moveDown({required bool focusToPreviousCell}) {
    ...
    if (!focusToPreviousCell || stateManager.currentCell == null) {
      ...
      stateManager.scrollByDirection(TableMoveDirection.down, 0);
    }
  ...
  void _handleFocusFromRows(TableGridEvent tableEvent) {
    if (!_enabled) {
      return;
    }
