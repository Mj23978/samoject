...
/// of [TableViewCell] or [TableViewColumn], [TableViewColumnGroup] of [TableViewRow]
/// or render only the widgets displayed according to the screen width.
class TableVisibilityLayout extends RenderObjectWidget
    implements MultiChildRenderObjectWidget {
  const TableVisibilityLayout({
    super.key,
  ...
  @override
  final List<TableVisibilityLayoutId> children;
  ...
  @override
  TableVisibilityLayoutRenderObjectElement createElement() =>
      TableVisibilityLayoutRenderObjectElement(
        widget: this,
...
class TableVisibilityLayoutRenderObjectElement extends RenderObjectElement
    implements MultiChildRenderObjectElement {
  TableVisibilityLayoutRenderObjectElement({
    required TableVisibilityLayout widget,
    required this.scrollController,
    this.initialViewportDimension = 1920,
    this.textDirection = TextDirection.ltr,
  ...
  Iterable<TableVisibilityLayoutId> get _widgetChildren {
    if (textDirection == TextDirection.ltr) {
      return (widget as TableVisibilityLayout).children;
    }

    return (widget as TableVisibilityLayout).children.reversed;
  }
  ...
  bool visible({
    ...
    required TableVisibilityLayoutChild layoutChild,
  }) {
  ...
  Element? findChildByLayoutId(Object layoutId) {
    return _children.firstWhereOrNull((element) {
      if (element.widget is TableVisibilityLayoutId) {
        return (element.widget as TableVisibilityLayoutId).id == layoutId;
      }
      return false;
    });
  }
  ...
  void performRebuild() {
    ...
    for (final child in _children) {
      ...
      final layoutChild = (child.widget as TableVisibilityLayoutId).layoutChild;
  ...
  @override
  void update(TableVisibilityLayout newWidget) {
...
class TableVisibilityLayoutId extends LayoutId {
  TableVisibilityLayoutId({
    Key? key,
    required super.id,
    required TableVisibilityLayoutChild child,
  }) : super(key: key, child: child);

  TableVisibilityLayoutChild get layoutChild =>
      child as TableVisibilityLayoutChild;
}

abstract class TableVisibilityLayoutChild implements Widget {
  double get width;
