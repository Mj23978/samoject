...
enum TableResizeMode {
  none,
  normal,
  pushAndPull;

  bool get isNone => this == TableResizeMode.none;
  bool get isNormal => this == TableResizeMode.normal;
  bool get isPushAndPull => this == TableResizeMode.pushAndPull;
}
...
enum TableAutoSizeMode {
  none,
  equal,
  scale;

  bool get isNone => this == TableAutoSizeMode.none;
  bool get isEqual => this == TableAutoSizeMode.equal;
  bool get isScale => this == TableAutoSizeMode.scale;
}

/// Returns the auto-sizing class according to
/// [TableAutoSizeMode.equal] or [TableAutoSizeMode.scale].
///
...
/// If [mode] is [TableAutoSizeMode.scale], you must pass the value of [scale].
...
/// {@endtemplate}
class TableAutoSizeHelper {
  static TableAutoSize items({
    required double maxSize,
    required int length,
    required double itemMinSize,
    required TableAutoSizeMode mode,
    double? scale,
  }) {
    if (mode.isScale) assert(scale != null);

    switch (mode) {
      case TableAutoSizeMode.equal:
        return TableAutoSizeEqual(
          maxSize: maxSize,
          length: length,
          itemMinSize: itemMinSize,
        );
      case TableAutoSizeMode.scale:
        return TableAutoSizeScale(
          maxSize: maxSize,
          length: length,
          scale: scale!,
          itemMinSize: itemMinSize,
        );
      case TableAutoSizeMode.none:
        throw Exception('Mode cannot be called with TableAutoSizeMode.none.');
    }
  }
}

abstract class TableAutoSize {
...
class TableAutoSizeEqual implements TableAutoSize {
  TableAutoSizeEqual({
...
class TableAutoSizeScale implements TableAutoSize {
  TableAutoSizeScale({
    required this.maxSize,
    required this.length,
    required this.scale,
...
///
/// Cannot be called with [TableResizeMode.none] or [TableResizeMode.normal] .
///
...
/// {@endtemplate}
class TableResizeHelper {
  static TableResize items<T>({
    ...
    required TableResizeMode mode,
  }) {
    switch (mode) {
      case TableResizeMode.pushAndPull:
        return TableResizePushAndPull<T>(
          offset: offset,
          items: items,
      ...
      case TableResizeMode.none:
      case TableResizeMode.normal:
...
/// that must be inherited when implementing the class according to [TableResizeMode].
///
/// {@macro resize_helper_params}
abstract class TableResize<T> {
  TableResize({
...
class TableResizePushAndPull<T> extends TableResize<T> {
  TableResizePushAndPull({
